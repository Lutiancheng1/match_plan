  ...
  <script>
    ...
    function renderStatus(status, data) {{
      ...
      const rows = [
        ...
        ["最后成功", status.last_success || "尚未成功抓取"],
...
def load_live_inputs(args: argparse.Namespace) -> tuple[str, str | None, dict[str, str], list[str], str, str]:
    ...
    if not body or not cookie:
        ...
        if login_user and login_pass:
            print(f"Auto-login as {login_user}...", file=sys.stderr)
            creds = auto_login.auto_login(login_user, login_pass, entry_url)
            cookie = creds["cookie"]
            body = creds["body_template"]
            print(f"Auto-login OK: uid={creds['uid']}", file=sys.stderr)
        elif not body:
            raise SystemExit(
    ...
    *,
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            # Detect session-killed errors (e.g. doubleLogin) and force re-login
            need_relogin = False
            for gtype_code, feed in payload.get("feeds", {}).items():
            ...
            if need_relogin and login_user and login_pass:
                print("[polling] doubleLogin detected, re-logging in...", file=sys.stderr)
                try:
                    creds = auto_login.auto_login(login_user, login_pass, entry_url)
                    cookie = creds["cookie"]
                    ...
                    print(f"[polling] re-login OK: uid={creds['uid']}", file=sys.stderr)
                except Exception as login_err:
                    status["last_error"] = f"re-login failed: {login_err}"
                    print(f"[polling] re-login FAILED: {login_err}", file=sys.stderr)
            else:
                ...
                if db_conn is not None:
                    try:
                        ...
                        print(f"[db] write error: {db_err}", file=sys.stderr)
            ...
            if (consecutive_failures >= max_failures_before_relogin
                ...
                print(f"[polling] {consecutive_failures} consecutive failures, re-logging in...", file=sys.stderr)
                try:
                    creds = auto_login.auto_login(login_user, login_pass, entry_url)
                    cookie = creds["cookie"]
                    template = poller.parse_form_body(creds["body_template"])
        ...
        if cleanup_counter >= 60:
            ...
            if deleted:
                print(f"[cleanup] deleted {deleted} old snapshot files", file=sys.stderr)
...
def main() -> int:
    ...
    env_path = os.environ.get(
        "LIVE_ENV",
        str(Path(__file__).resolve().parent / "live_dashboard.env"),
    )
    ...
    if os.environ.get("DB_ENABLED", "1") == "1":
        ...
        if deleted_files:
            print(f"[startup] cleaned {deleted_files} old snapshot files", file=sys.stderr)
        stats = db_store.get_db_stats(db_conn)
        print(f"[db] history.db ready: {stats['snapshot_count']} snapshots, {stats['db_size_mb']}MB", file=sys.stderr)
    ...
    worker.start()
    print(json.dumps({"url": server_url, "output_dir": str(outdir)}, ensure_ascii=False))
    try:
